...
export const DatePickerModal: React.FC<DatePickerModalProps> = ({
    ...
    const adjustDayIfNeeded = useCallback((month: number, year: number, currentDay: number) => {
    ...
    const handleMonthChange = useCallback((monthIndex: number) => {
        ...
        const adjustedDay = adjustDayIfNeeded(monthIndex, selectedYear, selectedDay);
    ...
    }, [selectedMonth, selectedDay, selectedYear, isDateValid, onConfirm, modalScale, overlayOpacity]);
    ...
    useEffect(() => {
        if (visible) {
            ...
            setTimeout(() => {
                const initial = getInitialValues();
                monthScrollRef.current?.scrollTo({ y: initial.month * ITEM_HEIGHT, animated: false });
                dayScrollRef.current?.scrollTo({ y: (initial.day - 1) * ITEM_HEIGHT, animated: false });
